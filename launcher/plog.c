...
static gboolean plog_check(GSource *state)
{
...
static gboolean plog_dispatch(GSource *state, GSourceFunc callback, gpointer user_data)
{
    struct plog_source *source = (void *)state;
    WebKitWebView *web_view = (WebKitWebView *)source->view;
    g_print("plog_dispatch\n");
...
static GSourceFuncs plog_funcs = {
    .check = plog_check,
    .dispatch = plog_dispatch,
};
...
static void on_message(WebKitUserContentManager *content, WebKitJavascriptResult *result, void *view)
{
    ...
    struct plog_source *source = (void *)g_source_new(&plog_funcs, sizeof(struct plog_source));
    source->view = view;
    source->pid = pid;
...
static void on_activate(GApplication *self, void *user_data)
{
    ...
    //WebKitWebContext *webctx = cog_shell_get_web_context(shell);
    //CogView *view = cog_viewport_get_visible_view(viewport);
